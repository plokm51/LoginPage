...
$(document).ready(function () {
    ...
    if (userNameIsV && passwordIsV) {
      ...
      $.get('http://localhost:3000/users', function (data, status) {
        ...
        if (user) {
          ...
          $('.message-container .message-text').text('Welcome')
          ...
          $('.message-container .message-text').text('Error')
          $('.message-container').show()
          console.log('Error')
        }
      })
  ...
  $('.button-register').on('click', function (e) {
    ...
    if (registerPhoneIsV && registerUserNameIsV && registerPasswordIsV && registerPassword2IsV) {
      ...
      var password = $('.register-password').val()
      console.log('start registration 1')
      $.post('http://localhost:3000/users', {
        'user-name': userName,
        'phone-number': phoneNumber,
        password: password
      }, function (data, status) {
        console.log('start registration 2')
        console.log(data, status)
        // var user = findUser(data, userName, password)
        if (status === 'success') {
          $('.message-container .icon-true').css('background-image', 'url("img/正确.png")')
          $('.message-container .message-text').text('registration success')
          $('.message-container').show()
          ...
          $('.message-container .icon-true').css('background-image', 'url("img/错误.png")')
          $('.message-container .message-text').text('registration failed')
          $('.message-container').show()
      ...
      $('.message-container .icon-true').css('background-image', 'url("img/错误.png")')
      $('.message-container .message-text').text('input error')
      $('.message-container').show()
      console.log('unable to register', registerPhoneIsV, registerUserNameIsV, registerPasswordIsV, registerPassword2IsV)
  ...
  $('.input-remember').on('change', function () {
    if ($('.input-remember')[0].checked) {
      console.log('record cookie', $('.input-user-name').val(), $('.input-password').val())
      $.cookie('uu', $('.input-user-name').val(), { expires: 7 })
      $.cookie('pp', $('.input-password').val(), { expires: 7 })
      console.log($.cookie('uu'), $.cookie('pp'))
    } else {
      console.log('no record cookie', $('.input-user-name').val(), $('.input-password').val())
      $.cookie('uu', '')
      $.cookie('pp', '')
    }
  })
